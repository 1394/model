...
function createPool(cfg, debug){
  ...
  if(cfg.default=='true'){
    internals.default = cfg.database
  }
...
internals.getPool = function getPool(db){
  return this.pools[db] || this.pools[this.default] || throw new Error('cant find pool for database : %s , please check config or define default database in common config or database config!',db)
}.bind(internals)
