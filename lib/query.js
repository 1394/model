...
class Query {
  ...
  field(f) {
    ...
    console.log('push fields result', this.chain.get('fields'))
    return this
  }
  ...
  distinct(distinct) {
    if (typeof distinct === 'string') {
      this.field_set(`DISTINCT ${distinct}`)
    }
    return this
  }
  ...
  _doInsert() {
    ...
    const keys = Object.keys(isFieldArray ? fieldData[0] : fieldData).map(me.wrapDot)
    const vals = (isFieldArray ? fieldData : [fieldData]).map((el) => convertVals(keys, el))
    const r = [`INSERT INTO ${table}${Array.isArray(tables) ? ','+tables.map(me.wrapDot).join(',') : ''}`]
    r.push(`(${keys.join(',')})`)
