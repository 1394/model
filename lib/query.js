...
const getDate = (d) => df(d, 'yyyy-mm-dd hh:mm:ss')
...
const convert = (val, {withEscapeString} = {}) => {
  ...
  const toString = (str) => withEscapeString ? `\'${escapeString(str)}\'` : escapeString(str)
  if (val instanceof Date) {
    return toString(getDate(val))
  }
...
class Query {
  ...
  order(field, dir) {
    ...
    if (!dir) {
      dir = ['asc', 'desc', 'ASC', 'DESC'].find((el) => field.includes(el))
      if (dir) {
        field = field.split(dir)[0]
        dir = dir.toUpperCase()
      } else {
        dir = 'ASC'
      }
