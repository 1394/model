...
const setupPool = (cfg) => {
  ...
  if (!internals.pools[cfg.database]) {
    internals.pools[cfg.database] = internals.driver.createPool(cfg)
  }
  return internals.pools[cfg.database]
}
...
class DBService {
  constructor(cfg, debug, options) {
    ...
    this.debug = debug
    this.pool = setupPool(this.cfg)
    if (options.serviceConn && !this.cfg.testOnly) {
  ...
  mockConn(conn) {
    if (this.cfg.testOnly) {
      return {
        query: (...args) => console.log('\n*TEST.MODE* DBService: call conn.query(args) => ', ...args, '\n') && [],
        release: () => console.log('\n*TEST.MODE* DBService: conn.release()\n'),
      }
    }
  }
  ...
  detectMode(sql) {
    if (sql.trim().toLowerCase().startsWith('select')) {
      return {sql, mode: 'select'}
    }
  }

  doConn(sConn, opts) {
    const testConn = this.mockConn()
    const conn = testConn || sConn
    const me = this
  ...
  async do(opts) {
    ...
    const conn = testConn || await this.getConn()
    let result
