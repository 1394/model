...
if (mode === 'up' && !name) {
  ...
  let check = async function () {
    ...
    for (let module of files.map(el => el.name).sort()) {
      try {
        let fn = require('./migrations/' + module)
        console.log('----------------')
        await fn.up()
...
if (mode === 'down' && !name) {
  ...
  let check = async function () {
    ...
    let file = require('glob').sync('./migrations/*.js').map(file => path.parse(file)).filter(el => rec.name === el.name)[0]
    if (!file) {
      console.error(`the file 'migrations/${rec.name}'.js needed for the migration is not found`)
      process.exit(1)
    }
    try {
      let fn = require('./migrations/' + file.name)
      await fn.down()
...
if (mode === 'create' && name) {
  ...
  let migrationFile = path.join(process.cwd(), 'migrations', fileName + '.js')
